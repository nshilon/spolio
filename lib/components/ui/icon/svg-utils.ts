  ...
  const reactElements = childNodes
    ...
    .map((node, index) => {
      ...
      Array.from(element.attributes).forEach(attr => {
        ...
        const attrName = attr.name.replace(/-([a-z])/g, (_, letter) => letter.toUpperCase());
 ...
 */
export async function loadAndRegisterSvgIcon(name: string, url: string): Promise<void> {
  try {
 ...
 */
export async function loadAndRegisterSvgIcons(icons: Record<string, string>): Promise<void> {
